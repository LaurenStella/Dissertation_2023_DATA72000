...
assert np.array_equal(df_full.loc["2006-01-07",:,:][label_col].values,
                      df_full.loc["2006-01-12",:,:][label_col_lag5].values)
...
print("Saving best Autoregression-5 Linear Regression model as '5-Autoreg_linear.pickle' file")
pickle.dump(reg, open("5-Autoreg_linear.pickle", "wb"))
...
print("Saving test & y_pred column to 'y_pred_5-autoreg_linear.csv'")
observe_test.to_csv('y_pred_5-autoreg_linear.csv')

##ONLY NEED TO SAVE THIS ONCE (PER NUMBER OF LAGS), NOT FOR EACH MODEL
#observe_train = df_train.iloc[:,:5]
#print("Saving train to 'train_label_autoreg-5.csv'")
#observe_train.to_csv('train_label_autoreg-5.csv')