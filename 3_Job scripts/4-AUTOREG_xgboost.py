...
label_col_lag4 = [f+"_lag_4" for f in label_col]
...
assert np.array_equal(df_full.loc["2006-01-06",:,:][label_col].values,
                      df_full.loc["2006-01-10",:,:][label_col_lag4].values)
...
print("Saving best Autoregression-2 XGBoost model as '4-Autoreg_xgboost.pickle' file")
pickle.dump(automl, open("4-Autoreg_xgboost.pickle", "wb"))
...
print("Saving test & y_pred column to 'y_pred_4-autoreg_xgboost.csv'")
observe_test.to_csv('y_pred_4-autoreg_xgboost.csv')
