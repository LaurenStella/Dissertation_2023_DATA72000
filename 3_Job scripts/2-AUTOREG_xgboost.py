...
label_col_lag2 = [f+"_lag_2" for f in label_col]
...
num_lags = 2 ##Set number of lags
print("Number of lags:", num_lags)

df_full = lag_label(df_full, num_lags)
display(df_full)

##Checking adding 2 lag days has worked correctly - IT HAS!:)
assert np.array_equal(df_full.loc["2006-01-04",:,:][label_col].values,
                      df_full.loc["2006-01-06",:,:][label_col_lag2].values)
...
print("Saving best Autoregression-2 XGBoost model as '2-Autoreg_xgboost.pickle' file")
pickle.dump(automl, open("2-Autoreg_xgboost.pickle", "wb"))

##Best estimator
print('Best XGBoost learner:', automl.best_estimator)
...
print("Saving test & y_pred column to 'y_pred_2-autoreg_xgboost.csv'")
observe_test.to_csv('y_pred_2-autoreg_xgboost.csv')

##ONLY NEED TO SAVE THIS ONCE (PER NUMBER OF LAGS), NOT FOR EACH MODEL
#observe_train = df_train.iloc[:,:5]
#print("Saving train to 'train_autoreg-3.csv'")
#observe_train.to_csv('train_autoreg-3.csv')