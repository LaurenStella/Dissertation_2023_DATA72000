...
feature_cols_lag2 = [f+"_lag_2" for f in feature_cols]
...
assert np.array_equal(df_full.loc["2006-01-04",:,:][feature_cols].values,
                      df_full.loc["2006-01-06",:,:][feature_cols_lag2].values)
...
print("Saving best Autoregression-2 Linear Regression model as '2-Lagged_full_linear.pickle' file")
pickle.dump(reg, open("2-Lagged_full_linear.pickle", "wb"))
...
print("Saving test & y_pred column to 'y_pred_2-lagged_full_linear.csv'")
observe_test.to_csv('y_pred_2-lagged_full_linear.csv')

##ONLY NEED TO SAVE THIS ONCE (PER NUMBER OF LAGS), NOT FOR EACH MODEL
#observe_train = df_train.iloc[:,:5]
#print("Saving train to 'train_autoreg-2.csv'")
#observe_train.to_csv('train_autoreg-2.csv')